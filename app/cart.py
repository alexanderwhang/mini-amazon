...
class Cart:
    def __init__(self, id, uid, pid, name, imageurl, price, quantity, time_added_to_cart):
        self.id = id
        self.uid = uid
        self.pid = pid
        self.name = name
    ...
    def get_all_by_uid(uid):
        rows = app.db.execute('''
SELECT Carts.id, uid, pid, name, imageurl, price, Carts.quantity, time_added_to_cart
FROM Carts, Products
WHERE uid = :uid
...
def cart(action=None, uid=None, pid=None, quantity=1):
    ...
    # the posting methods allow for customizability of user's cart including
    # deleting items, updating quantities, saving items for later
    if request.method == "POST":
        ...
        if action == 'save':
            ...
            # add item to saved for later page
